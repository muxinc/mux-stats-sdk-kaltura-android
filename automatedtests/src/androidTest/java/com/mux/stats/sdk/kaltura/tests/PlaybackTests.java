...
public class PlaybackTests extends TestBase {
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          ...
          customerVideoData.setVideoTitle(currentTestName.getMethodName()
              + "_title_2");
          MuxStatsKaltura muxStats = testActivity.getMuxStats();
          if (videoChange) {
  ...
  public void testEndEvents() {
    try {
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          long contentDuration = testActivity.getPlayer().getDuration();
    ...
    try {
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          ...
          testActivity.getPlayer().seekTo(currentPlaybackPosition / 2);
        }
      });
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          ...
          testActivity.getPlayer().seekTo(seekToInFuture);
        }
      });
